function attachDemoControls(tremula){
	...
	jQuery(".streamHorizontal").click(function() {
	...
	jQuery(".mountain").click(function() {
		s.toggleScrollAxis('x');
		setTimeout(function(){
			$body.addClass('doReflect');
	...
	jQuery(".more").on('click',function(evt) {
		evt.stopPropagation();
		jQuery('.streamHorizontal').click();
		$body.addClass('showTweaks')
		jQuery(".more").remove();
	})

	jQuery(".controls").on('click',function(evt) {
		evt.stopPropagation();
		if(jQuery(evt.target).hasClass('buttons')||jQuery(evt.target).hasClass('controls'))
			$body.removeClass('showControls');
	})

	jQuery(".showCtrlData").on('click',function(evt) {
		toggleDebug();
	})
	

	jQuery(".toggleReflect").click(function() {
		$body.toggleClass('doReflect');
	})
	

	jQuery(".toggleItemEase").click(function() {
		s.setItemEasing(!s.itemEasing);
	})
	
	
	jQuery(".toggleScrollAxis").click(function() {
		s.toggleScrollAxis();
	})
	
	
	jQuery(".toggleIsLooping").click(function() {
		s.toggleIsLooping();
	})


	refreshData = false;//setting this to true will replace data with new results; false will append data.
	jQuery(".toggleRefreshData").text('appending');

	jQuery(".toggleRefreshData").click(function() {
		...
		if(refreshData)
			...
			label = 'appending'
		jQuery(".toggleRefreshData").text(label);
	})




	jQuery(".tailScroll").click(function() {
		s.setLastTouchOrigin(s.gridDims[s.si]/2);
		s.easeTo(s.getScrollPos()-(s.getScrollViewDim()*.50),800)
	})
	
	
	jQuery(".headScroll").click(function() {
		s.setLastTouchOrigin(s.gridDims[s.si]/2);
		s.easeTo(s.getScrollPos()+(s.getScrollViewDim()*.50),800)
	})
...
// console.log(newScrollOffset)
	...
	jQuery(".autoSize").click(function() {resizeFn = autoColumnSize; resizeFn(tremula); })
	jQuery(".autoCount").click(function() {resizeFn = autoColumnCount; resizeFn(tremula); })
	jQuery(".autoDisable").click(diableAuto)
	//jQuery(window).on('resize', $.debounce(250, function(){ resizeFn(tremula) }) );

	jQuery(".loadArtDotCom").click(function(){loadArtDotCom(null,postLoadSeq)});
	jQuery(".loadFlickr").click(function(){loadFlickr(null,postLoadSeq)})
	...
	function loadArtDotCom(url_,cb){
		...
		// var dataUrl = 'decorProductSearch.json';
			jQuery.getJSON(dataUrl,function(res){
