...
if ( ! class_exists( 'BFI_Image_Editor_GD_1_3' ) ) {
	...
	class BFI_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
	    ...
	    public function negate() {
	        ...
	        return new WP_Error( 'image_negate_error', esc_html__( 'Image negate failed.', 'mycity' ), $this->file );
	    }
	}
}
...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
	...
	class BFI_Thumb_1_3 {
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
	        ...
	        $basename = mycity_newBasename( $img_path );
	        ...
	        $dst_rel_path = str_replace( '.' . $ext, '', mycity_newBasename( $img_path ) );
...
function mycity_get_thumbnail($theid) {
	$thumbnail = get_the_post_thumbnail($theid, 'mycity_panorama');
...
function  mycity_get_big_img($post_id){
